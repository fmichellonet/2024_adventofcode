...
public class Day2RedNosedReportsPart1
{
    ...
    protected static IReadOnlyCollection<Report> Parse(string input)
    {
    ...
    private static Report Map(string line)
    {
        return new Report(Map(line.Split(" ", StringSplitOptions.RemoveEmptyEntries)));
    }

    private static IList<int> Map(string[] levels)
    {
        return levels.Select(int.Parse)
            .ToList();
    }

    public static long ComputeSafeReport()
    {
        var count = Parse(Input)
            .Select(x => x.Visit(0))
            .Count(x => x is { IsSafe: true, ErrorCount: 0 });

        return count;
    }


    protected record VisitedReport(IReadOnlyCollection<int> Levels, bool IsSafe, int ErrorCount);

    protected record Report(IList<int> Levels)
    {
        public VisitedReport Visit(int maxErrorCount)
        {
            return Visit(Levels, maxErrorCount, 0);
        }

        private static VisitedReport Visit(IList<int> levels, int maxErrorCount, int attempt, IReadOnlyCollection<int>? originalLevels = null)
        {
            var origLevels = originalLevels ?? levels.ToArray();

            if (attempt > maxErrorCount)
            {
                return new VisitedReport(origLevels, false, attempt);
            }
            
            var globalIsIncreasing = (levels.ElementAt(0) - levels.ElementAt(1)) > 0;
            for (var i = 0; i < levels.Count - 1; i++)
            {
                var item = levels.ElementAt(i);
                var nextItem = levels.ElementAt(i + 1);
                var localIsIncreasing = (item - nextItem) > 0;
                if (localIsIncreasing != globalIsIncreasing)
                {
                    return VisitPermutations(levels, maxErrorCount, attempt, origLevels);
                }

                switch (Math.Abs(item - nextItem))
                {
                    case < 1:
                        return VisitPermutations(levels, maxErrorCount, attempt, origLevels);
                    case > 3:
            ...
            return new VisitedReport(origLevels, true, attempt);
        }

        private static VisitedReport VisitPermutations(IList<int> levels, int maxErrorCount, int attempt, IReadOnlyCollection<int> originalLevels)
        {
            var permutationResult = GenerateAllPermutations(levels.ToArray())
                .Select(x => Visit(x, maxErrorCount, attempt + 1, originalLevels));

            return permutationResult.Any(x => x.IsSafe)
                ? new VisitedReport(originalLevels, true, attempt + 1)
                : new VisitedReport(originalLevels, false, attempt + 1);
        }

        private static IReadOnlyCollection<IList<int>> GenerateAllPermutations(IReadOnlyCollection<int> levels)
        {
            return Enumerable.Range(0, levels.Count)
                .Select(idx =>
                {
                    var tmp = levels.ToList();
                    tmp.RemoveAt(idx);
                    return tmp;
                })
                .ToList();
    ...
    {
        var res = ComputeSafeReport();
        Assert.That(res, Is.EqualTo(526));
    }
}