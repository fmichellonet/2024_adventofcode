...
public class Day5PrintQueuePart1
{
    ...
    public static (Dictionary<int, IEnumerable<int>> orderingRules, IReadOnlyCollection<int>[] updates) Parse(string input)
    {
        var allLines = input.Split(Environment.NewLine);
        var orderingRules = allLines.TakeWhile(x => x.Contains('|'))
            .Select(MapOrderingRules)
            .GroupBy(x => x.Item1)
            .ToDictionary(x => x.Key, x => x.Select(x => x.Item2));

        var updates = allLines.SkipWhile(x => !string.IsNullOrWhiteSpace(x))
            .Skip(1)
            .Select(MapUpdates)
            .ToArray();
    ...
    private static IReadOnlyCollection<int> MapUpdates(string input)
    {
        return input.Split(",")
            .Select(int.Parse)
            .ToArray();
    ...
    private static Tuple<int, int> MapOrderingRules(string line)
    {
        var pages = line.Split("|");
        return new Tuple<int, int>(int.Parse(pages[0]), int.Parse(pages[1]));
    }
    ...
    public void ShouldComputeTheCorrectTotalForExample()
    {
        const string input = """
                             ...
                             97,13,75,29,47
                             """;
        var (orderingRules, updates) = Parse(input);
        var total = Validate(orderingRules, updates)
            .Where(x => x.IsValid)
            .Sum(x => x.MiddlePage);
    ...
    public void ShouldComputeTheCorrectTotal()
    {
        ...
        Assert.That(total, Is.EqualTo(4689));
    }

    public static IReadOnlyCollection<Update> Validate(Dictionary<int, IEnumerable<int>> orderingRules, IReadOnlyCollection<int>[] updates)
    {
        return updates
            .Select(x => Validate(orderingRules, x))
            .ToArray();
    }

    public static Update Validate(IReadOnlyDictionary<int, IEnumerable<int>> orderingRules, IReadOnlyCollection<int> pages)
    {
        for (var i = 1; i < pages.Count(); i++)
        {
            if (!orderingRules.ContainsKey(pages.ElementAt(i)))
            {
                continue;
            }
            if (orderingRules[pages.ElementAt(i)].Contains(pages.ElementAt(i - 1)))
            {
                return new Update(pages, false);
            }
        }
        return new Update(pages, true);
    }

    public record Update(IReadOnlyCollection<int> PageNumbers, bool IsValid)
    {
        public int MiddlePage => PageNumbers.ElementAt(PageNumbers.Count / 2);
    }
}
